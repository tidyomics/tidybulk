...
We may want to reduce the dimensions of our data, for example using PCA or MDS algorithms. `reduce_dimensions` takes a `SummarizedExperiment`, an assay name via `.abundance`, and a method (e.g., MDS or PCA), then adds the reduced dimensions to the object metadata.
...
airway.norm.MDS =
  airway.norm |>
  reduce_dimensions(.abundance = counts_scaled, method="MDS", .dims = 2)
...
airway.norm.PCA =
  airway.norm |>
  reduce_dimensions(.abundance = counts_scaled, method="PCA", .dims = 2)
```
</div>
<div class="column-right">
Standard procedure (comparative purpose)
